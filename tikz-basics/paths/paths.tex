...
\mlatex{clip} defines the region where graphics are permitted to appear. Only graphics located on the "inside" of the (closed) path are drawn. Note that Tikz statements are executed sequentially, so the \mlatex{clip} command will only apply to statements below it.
...
\begin{minted}{latex}
    ...
    % Abbreviated form
    \begin{tikzpicture}
        \clip (-1, 0) -- (1, 0) -- (1, 1.5) -- (-1, 1.5) -- cycle;
        \shade (0, 0) -- (1, 1) -- (-1, 2) -- cycle;
    \end{tikzpicture}
\end{minted}
